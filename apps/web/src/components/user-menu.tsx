	...
	return (
		<DropdownMenu>
			...
			<DropdownMenuTrigger>
				<Button
					variant="ghost"
					className="flex cursor-pointer items-center gap-2 rounded-full px-3 py-6 transition-colors hover:bg-white/10"
				>
					{/* Avatar */}
					<div className="flex h-8 w-8 items-center justify-center rounded-full bg-linear-to-br from-primary to-indigo-600 font-semibold text-sm text-white">
						{user.name?.[0]?.toUpperCase()}
					</div>

					{/* Name */}
					<span className="hidden font-medium text-sm text-white/90 sm:block">
						{user.name}
					</span>
				</Button>
			</DropdownMenuTrigger>
