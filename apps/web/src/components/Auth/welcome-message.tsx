...
export default function WelcomeMessage({ mode }: WelcomeMessageProps) {
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="show"
      className="max-w-md text-left space-y-10 text-white"
    >
      {/* Logo / Icon */}
      <motion.div variants={item} className="flex items-center space-x-4">
        <div className="w-20 h-20 bg-transparent rounded-xl backdrop-blur flex items-center justify-center">
      ...
      <motion.div variants={container} className="space-y-5">
        {(isSignUp
          ? [
              ["Smart Scheduling", "Avoid conflicts with intelligent timelines"],
      ...
      <motion.div
        variants={item}
        className="pt-6 border-t border-white/20 text-sm text-blue-100"
      >
